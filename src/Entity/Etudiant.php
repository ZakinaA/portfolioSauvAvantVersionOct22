...
class Etudiant
{
    ...
    /**
     * @ORM\OneToMany(targetEntity=RP::class, mappedBy="etudiant", cascade={"persist", "remove"})
     */
    private $rPs;
    ...
    /**
     * @ORM\OneToMany(targetEntity=NoteEpreuve::class, mappedBy="etudiant", cascade={"persist", "remove"})
     */
    ...
    public function addNoteEpreuve(NoteEpreuve $noteEpreuve): self
    {
        if (!$this->noteEpreuves->contains($noteEpreuve)) {
            $this->noteEpreuves[] = $noteEpreuve;
            $noteEpreuve->setEtudiant($this);
        }

        return $this;
    }

    public function removeNoteEpreuve(NoteEpreuve $noteEpreuve): self
    {
        if ($this->noteEpreuves->removeElement($noteEpreuve)) {
            // set the owning side to null (unless already changed)
            if ($noteEpreuve->getEtudiant() === $this) {
                $noteEpreuve->setEtudiant(null);
            }
        }
