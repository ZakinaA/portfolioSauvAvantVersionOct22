...
/*
     * Permet de supprimer un stage dont l'id est passée en paramètre
     */
    public function remove($idStage): Response
    {        
        $repository = $this->getDoctrine()->getRepository(Stage::class);
        $stage = $repository->find($idStage);
        if ($stage->getEtudiant()->getid() != $this->getUser()->getEtudiant()->getId()  ){
            throw $this->createAccessDeniedException();
        }
        ...
        return $this->redirectToRoute('etudiantStages');
    ...
    public function showAddSemaineStage(Request $request, $idStage, $numSemaine)
    {
        ...
        elseif ($formSemaine->isSubmitted() && $formSemaine->isValid() ){
            $semaine = $formSemaine->getData();
            $entityManager = $this->getDoctrine()->getManager();
    ...
    */
    public function listerStagesAAffecter(Request $request, $idNiveau): Response
    {    
        ...
        // on récupère les étudiants selon le niveau passé en paramètre et dont la promo est encore en cours
        $repository = $this->getDoctrine()->getRepository(Etudiant::class);
        $etudiants = $repository->findBy(
            ['niveau' => $idNiveau, 'promotion' => $promos]);

        //Création du formulaure avec liste déroulante des enseignants
        $formEns = $this->createForm(StageAffecterEnseignantType::class);
        $formEns->handleRequest($request);

        if ($formEns->isSubmitted()) 
        {

            // si au moins une case a été cochée
            if (isset ($_POST["stage"]))
            {
                $stagesChecked=$_POST["stage"];
                foreach ($stagesChecked as $idStage)
                {
                
                    // récup des data du formulaire avec liste enseignants
                    $stage = $formEns->getData();      
                    $enseignant=$stage->getEnseignant();
                  
                    // récup du stage en fonction de la cache cochée
                    $stage = $this->getDoctrine()
                    ->getRepository(Stage::class)
                    ->find($idStage);
            ...
            return $this->render('enseignant/listStagesAAffecter.html.twig', array('form' => $formEns->createView(), 'etudiants'=>$etudiants));          
        }
    ...
    public function createAttestation($idStage)
    {
        ...
        $nomPdf = $etudiant->getNom().$etudiant->getPrenom().'.pdf';
